...
class TCL(nn.Module):
    ...
    @torch.no_grad()
    def generate_masks(
        self, image, text_emb, kp_branch_text_emb=None, text_is_token=False, 
        apply_pamr=False, kp_w=0.3, with_bg=False,
    ):
        """
        ...
        if with_bg:
            image_feat = self.masker.image_encoder.decoder.image_proj(image_feat[1:])
            H, W = int(math.sqrt(image_feat.shape[0])), int(math.sqrt(image_feat.shape[0]))
            image_feat = rearrange(image_feat, "(H W) B C -> B C H W", H=H, W=W)
            image_feat = image_feat * 5 + 5
            bg_pred = torch.sigmoid(self.bg_block(image_feat, text_emb))
            masks = torch.cat([bg_pred, masks], dim=1)
